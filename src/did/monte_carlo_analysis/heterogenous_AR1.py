...
import pandas as pd
import os
from did.monte_carlo_analysis.synthetic_data_gen import _generate_heterogenous_ar1_data
...
def monte_carlo_heterogenous_AR1_ols(data_info):
    """
    Perform Monte Carlo simulations for a heterogenousgenous AR(1) model with placebo treatment 
    assignment.
    This function performs Monte Carlo simulations on the generated treated data to understand the 
    small-sample properties of the coefficient of Treatment using Ordinary Least Squares (OLS) 
    regression.
    ...
    Notes:
    ------
    - This function generates synthetic panel data using a heterogenous AR(1) model, assigns placebo 
      treatment to states, and performs Monte Carlo simulations to analyze the performance of OLS 
      estimators under different scenarios, namely without a 25% effect and with a 25% effect.
    ...
    Example:
    --------
    To perform Monte Carlo simulations with 5 states, 10 time periods and 100 individuals per state, 
    ...
    >>> results = monte_carlo_heterogenous_AR1_ols(data_dict)
...
def monte_carlo_heterogenous_AR1_crse(data_info):
    """
    Perform Monte Carlo simulations for a heterogenous AR(1) model with placebo treatment assignment.
    This function performs Monte Carlo simulations on the generated treated data to understand the 
    ...
    Notes:
    ------
    - This function generates synthetic panel data using a heterogenous AR(1) model, assigns 
      placebo treatment to states, and performs Monte Carlo simulations to analyze the performance 
      of CRSE estimators under different scenarios, namely without a 25% effect and with a 25% effect.
    - The treatment_variable is hard coded to be 'TREATMENT' because the _random_placebo_treatment 
      function generates a column called 'TREATMENT' which is subsequently used.
    ...
    >>> results = monte_carlo_heterogenous_AR1_crse(data_dict)
...
def monte_carlo_heterogenous_AR1_res_agg(data_info):
    """
    Perform Monte Carlo simulations for a heterogenous AR(1) model with placebo treatment assignment.
    This function performs Monte Carlo simulations on the generated treated data to understand the 
    ...
    ------
    - This function generates synthetic panel data using a heterogenous AR(1) model, assigns placebo 
      treatment to states, and performs Monte Carlo simulations to analyze the performance of 
      estimators under different scenarios, namely without a 25% effect and with a 25% effect.
    - The treatment_variable is hard coded to be 'TREATMENT' because the _random_placebo_treatment 
      function generates a column called 'TREATMENT' which is subsequently used.
    ...
    >>> results = monte_carlo_heterogenous_AR1_res_agg(data_dict)
...
def monte_carlo_heterogenous_AR1_wbtest(data_info):
    """
    Perform Monte Carlo simulations for a heterogenous AR(1) model with placebo treatment assignment.
    This function performs Monte Carlo simulations on the generated treated data to understand the 
    ...
    Notes:
    ------
    - This function generates synthetic panel data using a heterogenous AR(1) model,
      assigns placebo treatment to states, and performs Monte Carlo simulations to analyze
      the performance of wild cluster bootstrapping estimators under different scenarios, 
      namely without a 25% effect and with a 25% effect.
    - The treatment_variable is hard coded to be 'TREATMENT' because the _random_placebo_treatment
      function generates a column called 'TREATMENT' which is subsequently used.
    ...
    >>> results = monte_carlo_heterogenous_AR1_wbtest(data_dict)
...
def monte_carlo_heterogenous_AR1_fgls(data_info):
    """
    Perform Monte Carlo simulations for a heterogenous AR(1) model with placebo treatment assignment.
    This function performs Monte Carlo simulations on the generated treated data to understand the
    small-sample properties of the coefficient of Treatment using the Feasible Generalised Least
    Squares method of regression regression.
    The data is synthetically generated for each simulation, and placebo treatment is 
    randomly assigned in each run. The small sample statistics of the Treatment coefficient
    are stored in lists and compiled after the simulations are complete, to produce a DataFrame 
    ...
    data_info : dict
        ...
        - 'T' : int
            ...
            greater than 1.
        - 'num_individuals' : int
            The number of individuals or entities within each state or group. Must be a 
            positive integer.
        - 'num_simulations' : int, optional
            The number of Monte Carlo simulations to run.
        - 'alpha' : float, optional
            The significance level for hypothesis testing. Must be in the range (0, 1).
        - 'treatment_starting_period' : int, optional
            The starting period for assigning placebo treatment. Must be an integer not less 
            than the minimum time period in the data. Default is None, which assigns it to the
            minimum time period in the data.
        - 'treatment_ending_period' : int, optional
            The ending period for assigning placebo treatment. Must be an integer within the 
            data's time range and not less than treat_start_period if specified. Default is None, 
            which assigns it to the maximum time period in the data.

    Raises:
    -------
    ValueError:
        If any of the required parameters are missing or invalid. This function is created
        using the subfunctions. So if any of the parameters required by the subfunctions
    ...
    ------
    - This function generates synthetic panel data using a heterogenous AR(1) model, 
      assigns placebo treatment to states, and performs Monte Carlo simulations to analyze the 
      performance of estimator under different scenarios, namely without a 25% effect and with a 
      25% effect.
    - The treatment_variable is hard coded to be 'TREATMENT' because the _random_placebo_treatment
      function generates a column called 'TREATMENT' which is subsequently used.
    ...
    >>> results = monte_carlo_heterogenous_AR1_fgls(data_dict)
